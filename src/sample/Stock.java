...
//            if(i2 + 120 > end) i2 = end;
    ...
    public int getData2excel(String stock, String start, String end, HSSFSheet sheet, int counter) {
...
//        System.setProperty("http.proxyHost", "proxy1.edb.gov.hk");
//        System.setProperty("http.proxyPort", "8080");
try     {org.jsoup.nodes.Document doc = Jsoup.parse(Connection.cntAndGetString("https://finance.yahoo.com/quote/"+stock+"/history?period1="+start.toString()+"&period2="+end.toString()+"&interval=1d&filter=history&frequency=1d"));
        ...
        for (int i =  tr.size() -1 ; i >= 0; i--) {
            ...
            System.out.println(tr.get(i).text().length());
        if(tr.get(i).text().length()>12){
...
//
    ...
    catch(Exception e) {System.out.println("connection fail");}
    finally {
...
//            {stkrcds.remove(i);}
        ...
        {
            if(stkrcds.get(i-1).close > 0){
            System.out.println("date : "+stkrcds.get(i).date+ "  volumn : " + stkrcds.get(i).volumn + "  close_diff : " + stkrcds.get(i).close_diff + "  close : " + stkrcds.get(i).close);
            if (((stkrcds.get(i).close - stkrcds.get(i-1).close) / stkrcds.get(i-1).close) > 0.05) //define raising ratio
